...
if ($_SERVER['SERVER_PORT'] == '8888' || $_SERVER['HTTP_HOST'] == 'localhost' || $_SERVER['SERVER_PORT'] == '3000') {
...
function testPostToBokun($date, $apiKey, $signature, $path, $JSON)
{
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.bokuntest.com" . $path,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 30,
...
function popular()
{
    $getSignObjects = generateSignature("POST", "search");
    $jObjPop = array();
    $jObjPop->sortField = "BEST_SELLING_BY_VENDOR";
...
if (isset($_GET['updatetours']) && $_GET['updatetours'] == 'update') {
    ...
    foreach ($tours as $key => $value) {
        ...
        // var_dump($value->id);
        $jsonBokunIds[] = $value->id;
        $triplocalJsonId[] = array('bokun' => $value->id, 'wp' => '');
        ...
        foreach ($activetyAttributes as $kee => $activetyAttribute) {
            ...
            if (!term_exists($slug, 'tour')) {
                wp_insert_term($noSlug, 'tour', $args = array(
    ...
    foreach ($wpTours as $key => $tourToDelde) {
        ...
        $wpPostIds[] = $wp_post_id;
        $wp_bokun_id = $tourToDelde->acf->bokun_int_id;
        $a_bokun_id[] = $tourToDelde->acf->bokun_int_id;
        update_field('is_popular', 0, $tourToDelde->id);

        foreach ($tours as $key => $tourval) {
            if ($wp_bokun_id == $tourval->id) {

                $triplocalJsonId[$key]['wp'] = $tourToDelde->id;
                // var_dump($triplocalJsonId[$key]['wp']);
                // ->aId[] = array('bokun' => $value->id, 'wp' => '')
                // var_dump($tourToDelde->id);
            }
        }
    ...
    // ***** the big foreach wraps update and insert posts
    foreach ($jsonBokunIds as $key => $val) {

        // var_dump(get_page_by_title($tours[$key]->title, OBJECT,'tour_post_type' ));
        $thisWpPost = get_post($triplocalJsonId[$key]['wp'], OBJECT);
        $thisWpPostId = $thisWpPost->ID;
        ...
        if ($thisWpPostId && $jsonLastPublished > $thisWpPostDate) {
            ...
            foreach ($mymenu as $menuKey => $value) {
                ...
                if ($value->title == 'Trips') {
                    ...
                    //  var_dump($value);
                } elseif ($value->title == '#0 (no title)' || $value->title == '#0 (no title)') {
...
if (isset($_GET['pay']) && $_GET['pay'] == 'pay') {
    ...
    $phpAPIdata = testPostToBokun($getSignPayObjects[0], $getSignPayObjects[1], $getSignPayObjects[2], $getSignPayObjects[3], $_POST[0]);
    // $phpAPIdataItem = $phpAPIdata->items;
    // $jasonAPIdata = json_encode($phpAPIdata);
    print_r($phpAPIdata);
