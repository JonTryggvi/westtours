...
if (bknEngInput) {
  ...
  var rawAmount = { rawAmount: sumTotal };
...
$(document).click(function(e) {
  ...
  if (e.target.classList.contains('clickAgain')) {
    ...
    rawAmount.title = tripPostData.tourTitle;
...
$('#searchNations').on('keyup', function() {
  ...
  $.getJSON('https://restcountries.eu/rest/v2/name/' + that.val(), function(data) {
    ...
    $.each(data, function(key, value) {
      // console.log(JSON.stringify(value.flag));
      listOptions += '<div style="background-image:url(' + JSON.stringify(value.flag) + ');" value="' + value.name + '">' + value.name + '</div>';
    });
