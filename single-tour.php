...
?>
  <div class="tourModal">
    ...
    <div id="modalSlides" class="owl-carousel tourModal__slides">
      <?php if ($tourGallery): foreach ($tourGallery as $image): ?>
         <div class="tourModal__slides__slide" data-interchange="[<?php echo $image['sizes']['fp-small'] ?>, small], [<?php echo $image['sizes']['fp-medium'] ?>, medium], [<?php echo $image['sizes']['fp-large'] ?>, large], [<?php echo $image['sizes']['fp-large'] ?>, xlarge], [<?php echo $image['sizes']['fp-retina'] ?>, xxlarge]"></div>;
	...
	<article <?php post_class('main-content');?> id="post-<?php the_ID();?>" data-tourid="<?php echo $tourId; ?>">
      ...
      <?php endif;?>
        ...
        <?php else: ?>
            <section id="postImage" class="post-image" style="background-image:url(<?php echo $bokunImg; ?>)">
        <?php endif; ?>
...
if (!$filter): ?>
          ...
          <?php endif;?>
            ...
            <li><strong><?php echo $transYouths; ?> <?php $youthFix = (int) $minAdult - 1;
echo $minChild . '-' . $youthFix;?></strong><br>ISK <?php echo number_format($costChild, 0, ',', '.') . ' ' . $transVat; ?></li>
            <li><strong><?php echo $transChildren; ?> 0-<?php $childFix = (int) $minChild - 1;
echo $childFix;?></strong><br><?php ;if ($costInfant == "0" || $costInfant == null): echo $transFree;else:echo 'ISK ' . number_format($costInfant, 0, ',', '.') . ' ' . $transVat;endif;?></li>
...
<?php if (!$filter): $loop = new WP_Query(array('post_type' => 'wiki_post_type', 'posts_per_page' => 1, 'orderby' => 'rand'));
    $count = mt_rand(1, 2);
    $color;?>
